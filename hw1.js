...
const Circle = class Circle { 
    ...
    static area = (radius,color) =>{
        return calcArea;
    }

    static color = (radius,color) =>{
        return color;
    }

    calcArea(radius,color){
...
const Product = class Product {
   constructor(input){
      ...
      this.price = parseFloat(this.input.split(',')[1]);
      this.availability = this.input.split(',')[2];
    ...
    static halfOff = (products) => {
        ...
        return products.map((product) => {
           ...
           const upinput =  `${product.name},${halfPrice},${product.availability}`;
           return new Product(upinput);
        });
          return products;
